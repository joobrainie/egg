<template>
  <div class="space-y-4">
    ...
    <div v-if="isCuriosity" class="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm transition-all duration-300 hover:shadow-md">
      <button
        class="w-full px-5 py-4 bg-slate-50/50 flex justify-between items-center hover:bg-white transition-colors group"
        @click="saleExpanded = !saleExpanded"
      >
        <div class="flex items-center gap-3">
           <div class="w-8 h-8 rounded-xl bg-white border border-slate-200/50 shadow-sm flex items-center justify-center p-1.5 group-hover:scale-110 transition-transform">
             <div class="w-full h-full rounded bg-indigo-500 flex items-center justify-center text-white shrink-0">
               <span class="text-[8px] font-black tracking-tighter">70%</span>
             </div>
           </div>
           <h3 class="font-black text-[10px] uppercase tracking-[0.2em] text-slate-500">Research Sale</h3>
        </div>
        <svg
          class="w-5 h-5 text-slate-300 transition-transform duration-300"
          :class="{ 'rotate-180 text-slate-900': saleExpanded }"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
      ...
      <div v-show="saleExpanded" class="p-6 border-t border-slate-50">
        <WaitForResearchSale />
      </div>
    </div>
  </div>
...
import WaitForResearchSale from './WaitForResearchSale.vue';
...
const saleExpanded = ref(isCuriosity.value);
