  ...
  <div class="space-y-4">
    <div class="p-4 bg-slate-50 rounded-xl border border-slate-100 flex items-center justify-between">
      <div class="flex-1">
        ...
        <p
          ...
          class="text-[10px] font-bold text-purple-500 uppercase tracking-tight mt-1 flex items-center gap-1"
        >
      ...
      >
        ...
        <div
          class="absolute inset-0 bg-gradient-to-r from-purple-600 to-indigo-600 opacity-0 group-hover/btn:opacity-100 transition-opacity"
        ></div>
      </button>
    </div>
    ...
    <div class="px-4 py-3 bg-amber-50 rounded-xl border border-amber-100/50 flex gap-3">
      ...
      <p class="text-[10px] font-medium text-amber-700 leading-relaxed">
        This action will automatically adjust its duration if preceding actions are moved or modified, ensuring
        simulation ends exactly at the start of the next research sale.
      </p>
    </div>
  </div>
</template>
