      ...
      for (const r of researches) {
        ...
        for (let l = currentLevel; l < r.levels; l++) {
          ...
          const nextRState = {
            ...baseEngineState,
            population: Math.min(
              rSnapshot.habCapacity,
              rSnapshot.population + (rSnapshot.offlineIHR / 60) * timeDetails.timeToSave
            ),
            researchLevels: {
    ...
    if (currentView.value === 'roi') {
      ...
      const basicCandidates = uniqueUnpurchased.map(r => {
        ...
        return {
          ...
          totalRoiLabel:
            totalRoiSeconds === Infinity ? 'No Impact' : totalRoiSeconds < 0.1 ? '0s' : formatDuration(totalRoiSeconds),
          isLaying,
          isShipping,
          nextSnapshot,
